// Step3_DropoffLocations.jsx — Final Cleaned Version
import React, { useEffect, useState } from 'react';
import api from '../../../services/api';

export default function Step3_DropoffLocations({ vehicleId, formData, updateFormData, goNext, goBack }) {
  const [locations, setLocations] = useState([]);
  const [selectedIds, setSelectedIds] = useState(formData.dropoff_locations || []);
  ...
  const handleNext = async () => {
    const selectedDetails = locations.filter(loc => selectedIds.includes(loc.id));
        updateFormData({
        dropoff_locations: selectedIds,
        dropoff_locations_details: selectedDetails  
        });
    try {
      await api.patch(`/vehicles/${vehicleId}/`, { dropoff_locations: selectedIds });
    } catch (err) {
  ...
  return (
    <div className="vehicle-form">
      <h2 className="vehicle-heading">Dropoff Locations</h2>
